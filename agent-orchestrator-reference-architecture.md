...
- **Авторизация - двухуровневая.** Глобальные роли (admin / member / viewer) + пер-тенантные роли (owner / admin / member). Тонкость: **источник истины роли**. Если роль пересчитывается при каждом логине из конфига (список admin-email), то UI-кнопка «сделать админом» будет молча затираться при следующем входе. Reconciliation должна **только повышать**, не понижать выданное через UI, - иначе UI и конфиг конфликтуют.
...
**scion:** роли `admin/member/viewer` (глобально) + `owner/admin/member` (в проекте). Глобальный admin даётся из env-списка `admin_emails`, и логин-сверка перезаписывает роль по нему (тонкость выше - реальная). GCP-identity брокерится per-pod через metadata-interception в `sciontool` + токен-эндпоинт хаба; долгоживущих SA-ключей в образах нет.
**Зрелость:** ✅ tenancy, RBAC, изоляция, credential-брокеринг. 🟡 reconciliation ролей затирает UI-грант (дизайн-изъян). 🟡 per-project k8s-namespace изоляция требует патча.
...
**Семантика доставки:** at-least-once с идемпотентностью; **контракт топиков** должен совпадать у хаба и плагинов. Тонкость: устаревший out-of-process плагин, публикующий в старом формате топика, тихо ломает inbound (новый хаб отвергает «invalid topic»). Правило: пересобирать плагины вместе с хабом.
...
**Зрелость:** ✅ fan-out, in-process, Telegram, топик-скоупинг. 🟡 контракт топиков дрейфует (нужна дисциплина пересборки плагина). 🟡 A2A-мост есть, но не развёрнут (нужны конфиг + signing-key + публичный маршрут).
...
**Зрелость:** ✅ OTLP-релей, статус-модель, hub-метрики. 🟡 несоответствие имён метрик (`gen_ai.*` vs `claude_code.*`) даёт нули на дашборде - реальный gap. 🔴 eval-слой отсутствует.
...
**scion:** фазы агента + heartbeat-timeout reaper + stuck-message sweep; `resume` (и `resume --force` для восстановления упавших из error, напр. после вытеснения GKE-пода). Полноценного replay-движка нет - resume опирается на сохранённую сессию харнеса, а не на детерминированный реплей истории.
...
- **Безопасность.** Hardened non-root контейнеры, per-agent секреты, network policy, credential-брокеринг (без долгоживущих ключей), supply-chain (провенанс образов). Класс проблем «hardened non-root pod ломает git-операции из-за dubious ownership» - реальный (лечится доверием workspace до git-операций).
...
**scion:** брокеры - исходящий websocket (BYOC-дружелюбно); агенты **персистентные** (GKE Autopilot, помесячно за под) -> **idle-стоимость реальна**, warm-пулов нет, вытеснение переживается через resume. Стор - SQLite (Postgres заблокирован guard'ом в этой топологии). Модель-алиасы есть.
**Зрелость:** ✅ горизонтальный execution-скейл, BYOC-брокеры, модель-алиасы. 🟡 idle-reaper/эфемерный режим отсутствует. 🟡 стор упирается в SQLite (нет HA-пути в текущей топологии).
...
| RBAC | Глобальные + пер-тенантные роли | 🟡 | reconciliation затирает UI-грант |
| Изоляция исполнения | Namespace/узел на тенанта | 🟡 | общий namespace; per-project нужен патч |
...
| A2A-интероп | Экспон. агентов по стандарту | 🟡 | мост есть, не развёрнут |
| Логи/трейсы | OTel GenAI semconv | 🟡 | релей есть; метрики мисматчат имена |
...
**Анти-паттерны (наблюдавшиеся на практике)**
- Долгоживущие креды/ключи в образах.
- Трактовать «закоммичено» как «сохранено» (**commit != push**; незапушенное теряется при пересоздании).
- Протекающие однопользовательские допущения в мультитенантном коде.
