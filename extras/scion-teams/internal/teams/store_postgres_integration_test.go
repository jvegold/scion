...
func testPostgresDSN(t *testing.T) string {
	t.Helper()
	dsn := os.Getenv("TEAMS_TEST_POSTGRES_DSN")
	if dsn == "" {
		t.Skip("TEAMS_TEST_POSTGRES_DSN not set")
	}
	return dsn
}

// TestAdvisoryLock_DedicatedConnection verifies that the advisory lock is held
// on a dedicated connection that survives pool churn.
...
// returns true when called on the holding connection.
...
// holding connection server-side releases the lock.
