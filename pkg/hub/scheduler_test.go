...
// Harness-config and credential lookups on the agent-create path.
//
// These are a panic trap rather than a live code path, and the distinction
// matters: today the two template getters above return ErrNotFound, so
// resolveTemplate yields nil, so populateAgentConfig computes an empty harness
// config name and never reaches any of these. The tests pass because of that
// accident, not because the mock is complete.
//
// The moment anyone gives this mock a template that resolves — the obvious
// thing to do when writing the next dispatch_agent test — populateAgentConfig
// reaches handlers_agent_create_helpers.go:228 and calls
// GetHarnessConfigBySlug on the embedded nil store.Store, which segfaults. The
// failure is a SIGSEGV in autogenerated wrapper code, not a readable test
// failure, so the next person pays for this in debugging time.
//
// GetEnvVar, GetSecret and ListGCPServiceAccounts sit one step further along
// the same path: if the harness config resolves and declares
...
// TestDispatchAgentEventHandler_ResolvableTemplateDoesNotPanic arms the trap
// that mockScheduledEventStore's stubs defuse.
//
// Before those stubs, this test did not fail — it segfaulted. populateAgentConfig
// reached GetHarnessConfigBySlug on the embedded nil store.Store and the whole
// test binary died with a SIGSEGV inside an autogenerated method wrapper,
// taking every other test in the package with it.
//
...
func TestDispatchAgentEventHandler_ResolvableTemplateDoesNotPanic(t *testing.T) {
	...
	for _, a := range ms.agents {
		if a.Slug == "tmpl-worker" && a.ProjectID == "project-1" {
			created = a
			break
		}
	}
